  ...
  if (results.multiHandLandmarks && results.multiHandedness) {
    for (let index = 0; index < results.multiHandLandmarks.length; index++) {
      ...
      if(isRightHand==false){
		...
		medio_anular_dist=landmarks[16].x*100 - landmarks[11].x*100;
		menique_anular_dist=landmarks[20].x*100 - landmarks[15].x*100;
		console.log(pulgar_dist)
		...
		if(medio_anular_dist<-6.5){
            medio_anular_dist_state="abierto";
        }
        else{
            medio_anular_dist_state="cerrado";
        }
		if(menique_anular_dist<-6){
            menique_anular_dist_state="abierto";
        }
        else{
            menique_anular_dist_state="cerrado";
        }
        if(pulgar_dist>8.5){
            pulgar_state="arriba";    
        }
        ...
        if (pulgar_state=="abajo" && indice_state=="arriba" && medio_state=="arriba" && anular_state=="arriba" && menique_state=="arriba" && indice_medio_dist_state=="abierto" && indice_angle_state=="ok" && medio_anular_dist_state=="abierto" && menique_anular_dist_state=="abierto"){
